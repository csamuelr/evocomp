...
class DifferentialEvolution:
	...
	def __init__(self, ng, np, cr, f, evfunc, algorithm, obl):
		self.__ng         = ng
		self.__np         = np
		self.__cr         = cr
		self.__f          = f
		...
		self.__all_best_solutions_mean = []
	...
	def heap_sort(self, temp=False):

		heap = []

		if temp:
			for individual in self.__temporaryp:
				heappush(heap, individual)
			ordered = []
			while heap:
				ordered.append(heappop(heap))
			
			self.__temporaryp = deepcopy(ordered)
	...
	def evolve(self):
		...
		self.__all_solutions = []
		self.__all_best_solutions = []
		self.__all_best_solutions_mean = [[]] * self.__np
		...
		p = int((self.__ng * 5)/100)

		opposite_positions = []
		for i in range(0, self.__ng, p):
		...
		for g in range(self.__ng):
			...
			###########################
			# Opposite-baded Learning #
			###########################

			if self.__obl and g in opposite_positions:
				self.__temporaryp = population.opposite(self.__population)
				self.evaluate()

				for indv in self.__population:
					self.__temporaryp.append(indv)
				
				self.heap_sort(temp=True)

				self.__population.clear()
				
				for individual in self.__temporaryp[:self.__np]:
...
if __name__ == '__main__':
	...
	for cfit in all_solutions:
		all_best_mean.append(mean(cfit))
